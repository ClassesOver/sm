...
3. 复杂分析或用户要求保留代码时，可以像 coding agent 一样用 `workspace_write_file` 或 `workspace_apply_changes` 创建任意 Python 脚本；迭代已有脚本时按“读取和哈希 → 精确补丁 → 重新读取或执行”处理，不得用 Shell 绕过文件写入确认。`report_analyze_dataset` 无需确认；执行脚本时提交 `python3 <工作区相对脚本路径>` 及其参数组成的完整非空 Shell 命令并向标准输出写出分析结果，不得直接提交裸 Python 代码或用空字符串占位。模型根据每轮结果自行决定轮数，并参考剖析基线判断证据是否充分；若返回 `ok: false`，读取 `exitCode` 和 `output`，修正脚本或命令后以同一 `jobId` 发起下一轮，直到至少一轮返回 `ok: true`。不得把失败结果当作分析结论，也不得另行向用户提问。
