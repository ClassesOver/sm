...
3. 分析完全使用生产 Coding 工具：先用 `terminal` 检查文件和可用依赖；已有脚本的小范围修改优先使用 `patch` 的 `mode="replace"`，创建脚本或多文件修改使用 `patch` 的 `mode="patch"`。用 `terminal` 执行 Python、Shell 或当前 Daytona 工作区允许的其他命令，运行中使用返回的持久 `session_id` 调用 `process` 的 `poll/wait/write/submit/kill`。生产 Coding Toolkit 的六个工具均不要求确认。Report 层不限制分析命令、输出大小、执行轮次或分析方式；失败时读取 `output` 和 `exit_code`，修正脚本或命令后继续。不得把失败结果当作分析结论，也不得另行向用户提问。
...
7. 调用 `report_job_status`。只有状态为 `validated` 且产物没有 `changed` 时，才能把 Markdown、PDF 和主要数据产物路径连同最后一次修改后的成功验证回执提交给 `finish_task`；只有 `finish_task` 返回 `accepted` 才可报告完成。`rendered`、`validation_failed` 或 `artifact_changed` 都不代表完成。
