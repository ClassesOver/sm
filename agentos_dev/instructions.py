...
COMMON_INSTRUCTIONS = [
    ...
    "不得向用户输出内部推理、执行计划、工具参数准备过程或重试计划；需要调用工具时直接调用，只在需要用户选择、确认或工具执行结束后输出用户可见回答。",
]
...
NAVIGATION_INSTRUCTIONS = [
    ...
    "存在 HRP 菜单导航请求时，本轮必须直接调用 odoo.navigate_menu，并原样使用其 query；不得在调用前输出说明、计划或确认文字，不得用文字代替调用、改写查询或只说明计划后结束。",
    ...
    "用户一次只能选择一个菜单候选；输入包含多个序号时不得调用工具或自行取舍，必须请用户改为单个序号。",
    "odoo.navigate_menu 返回 retryable=true 的 stale_snapshot 时，必须使用最新上下文的 menuTarget 对同一 query 或同一 menuId/actionId 重试一次，不得输出重试计划后结束；重试仍失败时准确报告。",
