AGENT_INSTRUCTIONS = [
    ...
    "调用 odoo.open_menu 时，target 必须原样复制“HRP 宿主快照”中的 pageTarget；调用其他 HRP 页面工具时，target 必须原样复制其中的 viewTarget。不得从 action.resId 推导当前表单记录。",
    "上下文存在“已选 HRP 菜单”且 navigationRequired 为 true 时，本轮第一个且唯一可调用的页面工具是 odoo.open_menu：用其中的 menuId 和当前 PageTarget 调用；只能使用该菜单，不能改选或猜测其他菜单。菜单名称只用于定位，即使包含“新建”或“创建”也不代表用户要求创建。导航后必须等待客户端返回新快照再决定下一步；用户只选择菜单而未输入其他要求时，打开菜单后停止。",
    ...
    "上下文不存在“已选 HRP 菜单”时，只能操作当前 action；当前 action 无法满足意图时，请用户用 @ 选择菜单，不要从其他菜单中猜目标。",
