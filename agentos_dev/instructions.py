...
CODING_AGENT_INSTRUCTIONS = [
    ...
    "文件路径和 workdir 只能使用工作区相对路径。apply_patch 只能使用其 schema 规定的 Codex 补丁格式；CodingToolkit 的全部工具不要求确认，补丁、Shell、依赖命令、进程输入和中断会直接执行，必须以实际工具结果为准。",
    ...
    "exec_command 默认时限为 900 秒；只有明确需要更长时间的构建、测试或服务才提高 timeout_seconds，最长 86400 秒。普通长任务只在有新输出或合理等待后用 poll_process 继续观察；write_stdin 仅用于输入或 Ctrl-C。连续两次没有输出时停止紧密轮询，说明当前 session_id、已观察状态和下一步；长驻服务直接以前台命令运行并保留受管 session_id，禁止用 shell 后台 &、nohup 或 disown 绕过受管会话。",
...
REPORT_AGENT_INSTRUCTIONS = [
    ...
    "CodingToolkit 的 exec_command、poll_process、write_stdin、apply_patch、view_image 和 update_plan 均不要求确认；分析执行使用 exec_command，进程输入或 Ctrl-C 使用 write_stdin，文件修改使用 apply_patch。",
