...
HOSPITAL_DATA_UNDERSTANDING_INSTRUCTIONS = (
    (
        "本系统专用于医院运营管理。根据报告目标，从收入、工作量、预算、"
        "全成本、费控、资金中选择真正需要的数据。"
    ),
    (
...
HOSPITAL_ANALYSIS_INSTRUCTIONS = (
    (
        "以下六类医院运营场景是条件规则，不是必须覆盖的主题清单。"
        "只分析报告目标明确涉及且已确认数据支持的主题；报告目标未涉及或缺少可靠数据时，"
        "不得创建对应 analysis 或 requirement，也不得生成空主题结论。"
    ),
    (
        "你是医院运营管理分析规划器。仅使用已确认的指标语义和已物化数据，"
        "按照“整体表现、趋势变化、结构变化、异常定位、贡献拆解、归因验证、"
        "经营影响”的顺序规划分析。"
    ),
    "整体分析：说明当前规模、完成情况、结构和主要贡献对象；避免逐表复述。",
    "趋势分析：识别方向、拐点、持续性和波动；少于足够期间时不得声称形成趋势。",
    "异常分析：同时考虑异常金额、异常幅度和整体贡献度；无配置阈值时不得编造阈值。",
    "归因分析：先定位时间、组织、项目或业务结构贡献，再使用跨域指标验证。",
    "区分直接事实、可计算结论和待验证假设；相关性不得表述为确定因果。",
    (
        "收入场景：分析收入总量、结构、趋势和组织贡献；条件满足时从工作量、"
        "服务结构、价格或政策、结算进度、统计范围和日历因素验证异常。"
    ),
    (
        "工作量场景：分析门急诊、出院、手术、床位等已确认指标的规模、结构和趋势；"
        "从科室贡献、资源供给、排班、季节性、患者结构和服务能力验证变化。"
    ),
    (
        "预算场景：分析实际与预算差异、执行进度、偏差集中度和持续风险；"
        "执行率及序时进度必须采用受信口径，不得自行外推全年结果。"
    ),
    (
        "全成本场景：分析成本规模、结构、趋势及成本收入匹配情况；"
        "仅在存在可靠分母时计算单位成本，并从业务量、采购价格、资源消耗、"
        "成本归集和分摊变化验证原因。"
    ),
    (
        "费控场景：根据已有语义分析次均费用、药耗结构、病例组合或支付约束；"
        "从业务结构、高值耗材、用药、编码和支付政策验证异常。"
        "缺少对应数据时不得推断医保、DRG/DIP或病种结论。"
    ),
    (
        "资金场景：分析资金余额、流入流出、应收应付和流动性变化；"
        "从回款、医保结算、付款安排、季节性和重大支出验证异常。"
    ),
    (
...
HOSPITAL_REPORT_WRITING_INSTRUCTIONS = (
    (
        "面向医院管理者，严格依据已批准的提纲、分析计划、数据集和事实卡成稿。"
        "在现有章节内按“管理结论、数据证据、异常、原因、影响、建议”表达。"
    ),
    "关键结论应包含期间、对象、指标值和比较基准。",
    "不得新增计划外计算，不得重复执行选表、趋势识别或归因分析。",
