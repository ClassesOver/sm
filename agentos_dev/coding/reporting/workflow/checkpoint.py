...
class ProfileReadReceipt(StrictModel):
    ...
    dataset_id: str = Field(alias="datasetId", min_length=1, max_length=256)
    profile_pointer: str = Field(alias="profilePointer", min_length=1, max_length=1024)
    ...
    def create(
        ...
        *,
        ...
        payload = json.dumps(
            [dataset_id, profile_pointer, snapshot_hash, purpose],
            ensure_ascii=False,
            separators=(",", ":"),
        ).encode("utf-8")
        return cls(
...
class SectionWorkItem(StrictModel):
    ...
    def validate_projection(self) -> SectionWorkItem:
        ...
        known_receipts = {item.receipt_id for item in self.profile_read_receipts}
...
def reporting_phase_task_key(
    ...
    *,
    ...
    if revision < 1 or attempt < 0 or (phase == "section") != bool(section_code):
        raise ValueError("Reporting phase task identity 无效")
    payload = f"{workflow_run_id}:{revision}:{phase}:{section_code or ''}:{attempt}".encode()
