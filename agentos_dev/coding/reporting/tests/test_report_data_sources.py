...
from agentos_dev.coding.reporting.intake import ReportIntakeService
from agentos_dev.coding.reporting.providers import SourceDescription, metadata_fingerprint
...
def test_数据转换动作预算为600秒():
    ...
    assert data_source_runtime.MAX_PART_BYTES == 200 * 1024 * 1024
...
async def test_临时来源必须通过原生工具确认后才写入会话绑定(monkeypatch):
    ...
    monkeypatch.setattr(data_sources.asyncio, "to_thread", inline_to_thread)
...
def test_sql校验器不可用时失败关闭(monkeypatch):
    ...
    monkeypatch.setattr(data_sources, "sqlglot", None)
    monkeypatch.setattr(data_sources, "exp", None)
...
def test_sqlite_只读物化生成受限_csv分片(tmp_path, monkeypatch):
    ...
    connection.commit()
    connection.close()
    monkeypatch.setattr(data_source_runtime, "WORKSPACE_ROOT", tmp_path)

    result = data_source_runtime.materialize_local(
        {
...
def test_sqlite_物化按实际文件大小拆分且列名唯一(tmp_path, monkeypatch):
    ...
    connection.close()
    monkeypatch.setattr(data_source_runtime, "WORKSPACE_ROOT", tmp_path)
    monkeypatch.setattr(data_source_runtime, "MAX_PART_BYTES", 96)
