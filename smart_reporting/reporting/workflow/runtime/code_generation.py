...
def _bounded_unsigned_paths(value: Any) -> list[str]:
    if not isinstance(value, list):
        return []
    return [item for item in value if isinstance(item, str) and 0 < len(item) <= MAX_DIAGNOSTIC_PATH_LENGTH][:MAX_DIAGNOSTIC_UNSIGNED_PATHS]
...
def _code_failure_kind(diagnostic: Mapping[str, Any] | None) -> ThinkingFailureKind | None:
    ...
    if code in {"execution_output_error", "report_analysis_script_failed", "report_visualization_script_failed"}:
...
class ReportingCodeGenerationRunner:
    ...
    async def run(self, task_context: ReportingCodingTaskContext, workspace: HostReportingWorkspace, task_facts: Mapping[str, Any], *, run_context: RunContext, diagnostic: Mapping[str, Any] | None = None) -> CodeGenerationResult:
        async with self.registry.bind(task_context, workspace) as binding:
            ...
            try:
                ...
                if receipt is None:
                    raise ReportingError("report_code_generation_no_submission", "Coding Agent 未签发成功执行的 Python 脚本。")
            ...
            logger.info("report_code_generation_completed task_id={} path={} duration_ms={}", task_context.task_id, receipt.source_file.path, max(0, round((perf_counter() - started_at) * 1000)))
    ...
    def _agent_failure(error: Exception) -> ReportingError:
        if isinstance(error, ModelRateLimitError):
            return ReportingError("report_code_generation_rate_limited", "Coding Agent 模型调用受限，请稍后重试。", details={"statusCode": error.status_code})
    ...
    def _short_diagnostic(cls, diagnostic: Mapping[str, Any]) -> dict[str, Any]:
        ...
        for field in ("line", "offset", "size", "lineCount", "maxLineLength", "exitCode"):
            ...
            if isinstance(value, int) and not isinstance(value, bool) and abs(value) <= MAX_DIAGNOSTIC_POSITION:
                safe[field] = value
        output = details.get("output")
        if isinstance(output, str) and output:
            safe["output"], truncated = bounded_python_script_diagnostic(output, MAX_DIAGNOSTIC_OUTPUT_LENGTH)
