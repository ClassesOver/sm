...
REPORT_VISUALIZATION_AGENT_INSTRUCTIONS = [
    ...
    (
        "提交前逐项核对最终图：禁止提交空图、仅坐标轴或仅图例的图表；分类维度不得为未知、空字符串或重复占位标签，"
        "否则重做、改为表格或跳过。每个声明的指标或系列都必须实际绘制；标题、图例、坐标轴和分类标签须完整可读，"
        "中文不得显示为方框，数值标签不得相互遮挡。图中数值、单位、期间和预算执行率必须与冻结事实及正文一致；"
        "跨年同比必须按相同月份对齐，不得将两个年度序列首尾拼接。"
    ),
    (
