...
from smart_reporting.reporting.cli import drive_workflow
...
@pytest.mark.anyio
async def test_cli_run_is_registered_once_with_cli_identity() -> None:
    repository = Repository()
    current = runtime(repository, [])

    await current.start_run(
        "run-a",
        prepared(current, run_id="run-a", entrypoint="cli"),
    )
...
@pytest.mark.anyio
async def test_cli_and_managed_workflow_share_one_run_lifecycle() -> None:
    repository = Repository()
    current = runtime(repository, [])

    async def execute(_step_input: StepInput) -> StepOutput:
        return StepOutput(content={"report": "done"})
    ...
    assert repository.registrations[0]["entrypoint"] == "cli"
