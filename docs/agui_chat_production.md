...
需要支持用户不使用 `@` 直接要求打开菜单时，将 `odoo.navigate_menu` 加入
`enabled_commands`。该工具只导航当前目录中自身配置 action 的末级菜单：传 `query` 时
唯一匹配在同一次调用中直接打开，多候选或无结果只返回候选且不导航；用户明确选择后，
改为原样传回 `menuId` 和 `actionId`。两种输入不得混用，ID 也不得只传一项。菜单目录
独立于页面快照，普通 Run 不发送完整目录；只有原词搜索无结果后的紧邻续跑才按 128 KiB
预算发送不含导航 ID 的完整路径。目录不完整时智能体必须停止并要求用户使用 `@`，不能
基于残缺路径猜测。
