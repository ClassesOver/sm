...
- 模型只看到 `apply_analysis_patch(patch)`，不再生成或传递 `expected_sha256`。
...
- 工具集合只包含 `apply_analysis_patch`；
- 每次生成尝试只允许并强制一次 patch 调用；
- 没有已提交脚本时必须用 `/dev/null` 到签发脚本路径的 create diff；
- 已有已提交脚本时必须针对 Workflow 提供的现有内容生成 update diff；
- patch 只能影响当前签发的一个 `.py` 文件。

Agent 的普通文本响应不代表成功。Workflow 只接受一次成功工具调用产生的唯一
`FileIdentity`；未调用、调用失败、多次调用、写入其他路径或没有唯一回执都按本次代码生成失败
处理。
...
2. 从实际内容派生每个 update/delete 的 `before_sha256`；
3. 在临时 Git tree 中执行 `git apply --check` 和 `git apply`，构造候选变更；
4. 将包含 before/after 身份的规范化操作写入 durable write intent；
5. `aapply_changes` 在 workspace lock 内重新读取目标，并核对内部 `before_sha256` 后原子提交；
6. 若读取和提交之间发生变化，返回冲突，不覆盖新内容。
...
校验失败时，Workflow 读取当前脚本，将受限源码、失败诊断和原始缺口交给修复 Agent，再强制一次
update patch。修复不得改写 `requiresSupplementalEvidence` 或绕过事实缺口。
...
VisualizationPlanDraft(charts, warnings)
  -> 可视化代码 Agent -> apply_analysis_patch(charts.py)
  -> Workflow 读取并验证固定 charts.py
  -> Workflow 执行固定 charts.py
  -> 校验签发图表文件和元数据绑定
...
Workflow 读取已提交的 `charts.py`，向修复 Agent 提供精简诊断并强制一次 update patch。恢复后从
脚本校验和执行阶段重新开始。租约冲突、取消、超时、capability 无效、工作区不可用和文件身份变化
继续直接失败关闭。
...
Workflow 对工具回执指向的实际文件执行校验，而不是信任模型返回内容：
...
大小、长行、语法或策略失败均产生机器可读诊断并消耗一次代码生成尝试；不会触发结构化输出
downgrade。物理行限制直接阻断日志中出现的 26 万列单行源码。
...
- schema 测试：决策/计划可通过；`script`、`scriptPath`、`pythonSource` 和模型可见
  `expected_sha256` 被拒绝。
- Agent 装配测试：无 `output_schema`，只暴露并强制 `apply_analysis_patch`；普通文本、零调用、失败
  调用和多调用都不能推进 Workflow。
...
- 边界测试：128 KiB、64 KiB 和 8 KiB 行长的边界值通过，超出一个字节失败；超长单行永不执行。
