...
薄宿主机适配只覆盖 Workflow 的程序化能力：逻辑路径解析、二进制读取、SHA-256、普通文件检查、线程目录生命周期和图表产物读取。它不是 Agent Toolkit，也不实现 sandbox provider 协议。

## 目录与路径模型

新增绝对路径配置 `REPORTING_HOST_WORKSPACE_ROOT`。应用启动时解析并创建该根目录；空值、相对路径、普通文件和符号链接均拒绝启动。每个线程使用不可逆摘要目录：

```text
<REPORTING_HOST_WORKSPACE_ROOT>/threads/<thread-sha256>/
  analysis/
  charts/
  reports/
  tasks/<task-id>/
...
现有业务载荷中的逻辑路径继续使用 `/home/daytona/workspace/<relative>`，避免修改冻结事实、图表计划和已持久化状态的路径协议。宿主机适配只接受该前缀下的规范化相对路径，并映射到当前线程目录；绝对宿主机路径不进入模型业务载荷。

Agno `Workspace` 以 `REPORTING_HOST_WORKSPACE_ROOT` 为根。任务指令只暴露线程和任务的相对目录。CodeMode 创建 session 后，Workflow 首个 bootstrap cell 将 cwd 切换到当前线程目录，并设置 Matplotlib `Agg` 后端。两者访问同一批物理文件，不做上传、下载或跨运行时复制。

## Agent 装配

可视化代码 Agent 保留现有模型选择、thinking 策略和任务身份，工具改为：

```python
Workspace(
    root=settings.reporting_host_workspace_root,
    allowed=Workspace.ALL_TOOLS,
    confirm=[],
)

CodeMode(
    cwd=settings.reporting_host_workspace_root,
    allow_shell=True,
    allow_restart=True,
    snapshot=False,
    timeout=settings.model_timeout_seconds,
    max_output_chars=65_536,
    max_images_per_cell=8,
    max_image_bytes=10 * 1024 * 1024,
    max_kernels=settings.report_analysis_concurrency,
)
```
...
CodeMode 使用现有 `task_id` 派生唯一 session ID，避免同一线程的并发章节共享 Python namespace。生成、正式执行和本任务内修复复用同一 session；任务成功、降级、取消或异常退出时均在 `finally` 中调用异步 shutdown。
...
- 路径规范化拒绝 `..`、NUL、控制字符和超出线程目录的解析结果。
- 正式脚本必须使用 `matplotlib.use("Agg")`，不打开 GUI。
- PNG 必须存在、非空、可被 Pillow 解码，并满足现有尺寸与文件大小限制。
- 线程目录的创建和清理由 Workflow 生命周期触发；失败清理记录 Loguru warning，不覆盖原始任务结果。
- 完全宿主机模式不声称阻止恶意 Python 或 Shell 访问线程目录之外的文件。
...
关键日志使用 Loguru，并至少包含 `thread_id` 摘要、`task_id`、`section_code`、`code_mode_session_id`、阶段、耗时和错误码。正常路径记录 Workspace/CodeMode 初始化、脚本签发、正式执行、产物校验和 Kernel 关闭；失败路径记录进入修复或降级的原因。
...
宿主机集成测试使用临时绝对目录，运行一个读取冻结 JSON、生成 Matplotlib PNG 的真实 CodeMode 任务，验证脚本、PNG、视觉检查输入和提交回执均来自同一目录，并验证结束后无存活 Kernel。测试不使用网络、Daytona、local-sandboxd 或容器。
