...
`fieldInfo`、分组布局、仅含 `id/name` 的当前 action 对象、分组排序、明细排序以及
`context.export_way/expWay`；随后携带当前
...
紧随其后的单个序号回复支持 `1` 至 `8`、`一`至`八`，以及“选择 2”“第 2 个”“第二项”
等无歧义表达；只有候选项仍具有相同的 menu/action ID，且搜索结果的目录 ID 和版本未变化时，才转为
显式菜单选择。随后 React 只为首个页面动作公开 `odoo.navigate_menu`，并向宿主提供所选 `menuId` 和
`actionId`。缺失、越界、过期、已完成或包含多个序号的选择仍作为普通对话输入，绝不授权导航；多个
序号必须由用户改为单个序号后再执行。

对于明确的“打开”“进入”“导航到”或“跳转到”请求，如果目标精确匹配可见叶节点名称或完整路径，React
会添加不含 menu/action ID 的 `HRP 菜单导航请求`，并可把本轮客户端工具收窄为
`odoo.navigate_menu`。客户端不发送首工具约束，AgentOS 也不强制首工具或因首个事件是文本或其他工具
而终止运行。服务端只把合法 Odoo command 能力绑定给 command 成员，由 Team 根据用户意图委派。
问题、未知目标、歧义结果、过期目录和已完成的打开操作不会获得导航工具声明。
...
页面快照变化返回带最新快照的 `stale_snapshot`。当其 `retryable=true` 时，Team 必须在官方 external
execution 续跑中使用最新上下文的 `menuTarget` 对同一 query 或同一 ID 对重试一次；调用前不输出内部
执行规划。重试仍失败时停止并准确报告，不由浏览器自动重放工具。
